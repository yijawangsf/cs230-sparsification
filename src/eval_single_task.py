"""
adapted from
https://github.com/gortizji/tangent_task_arithmetic/blob/main/src/eval_single_task.py
"""
import json
...
for dataset in datasets:
    ...
    if args.load:
        ...
        load_dir = f"{args.save}/{dataset}Val"

    if args.finetuning_mode == "linear":
        pretrained_checkpoint = f"{load_dir}/linear_zeroshot.pt"
    else:
        pretrained_checkpoint = f"{load_dir}/zeroshot.pt"

    finetuned_checkpoint = (
        f"{load_dir}/linear_finetuned.pt"
        if args.finetuning_mode == "linear"
        else f"{load_dir}/finetuned.pt"
    )
